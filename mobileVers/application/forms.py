...
class EligibilityForm(forms.ModelForm):
    rent = forms.ChoiceField(
        choices=choices, widget=forms.RadioSelect(), label="Rent")
...
class MoreInfoForm(forms.ModelForm):
    ...
    dependentsBirthdate = forms.DateField(
        label="Their Birthdate", widget=forms.widgets.DateInput(attrs={'type': 'date'}))

    class Meta:
        model = MoreInfo
        fields = ['dependentsName',
                  'dependentsBirthdate', 'dependentInformation']
        widgets = {'dependentInformation': forms.HiddenInput(), }


class FilesInfoForm(forms.ModelForm):
    last4SSN = forms.DecimalField(
        label='Because you uploaded an Affordable Connectivity Program confirmation, please enter the last four digits of your SSN')
...
class programForm(forms.ModelForm):
    class Meta:
        model = programs
        # ebb_acf = Emergency Broadband Benefit AKA Affordable Connectivity Program
        fields = ['ebb_acf', 'Identification', 'leap',
                  'medicaid', 'freeReducedLunch', 'snap']
        labels = {
